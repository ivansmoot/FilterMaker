...
def create_file(pics, filter_id):
    ...
    if not (Path(Path.cwd() / 'template.data').exists() and Path(Path.cwd() / 'template.desc').exists()):
        return -1
    # 所有的图片也必须存在
    for pic in pics:
    ...
    if Path(Path.cwd() / filter_id).exists():
        rmtree(Path(Path.cwd() / filter_id))

    # 创建该文件夹及子文件夹
    Path(Path(Path.cwd() / filter_id)).mkdir()
    Path(Path((Path.cwd() / filter_id) / 'image')).mkdir()
    Path(Path((Path.cwd() / filter_id) / 'raw')).mkdir()

    # 把data和desc移动进去
    (Path(Path.cwd() / 'template.data').replace(Path((Path.cwd() / filter_id) / (filter_id + '.data'))))
    (Path(Path.cwd() / 'template.desc').replace(Path((Path.cwd() / filter_id) / (filter_id + '.desc'))))

    # TODO 把图片复制进去,注意是复制,不是移动

    # 如果滤镜文件已存在,先删除
    if Path(Path.cwd() / (filter_id + '.tpl')).exists():
        Path.unlink(Path.cwd() / filter_id + '.tpl')
    ...
    folder_to_zip(zip_file, Path(Path.cwd() / filter_id))
