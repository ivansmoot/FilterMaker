...
from python_scripts.main import mainUI
...
def btn_click(self, grid):
    ...
    btn_name = 'pic_choose_btn' + str(mainUI.total_row - 1)
    edit_name = 'filter_choose_edit_combine_btn' + str(mainUI.total_row - 1)

    names[btn_name] = QPushButton('选择一张图片')
    names[btn_name].clicked.connect(lambda: pic_choose_btn_click.btn_click(self, mainUI.total_row - 3))

    names[edit_name] = QLineEdit()
    names[edit_name].setMaxLength(20)
    names[edit_name].setPlaceholderText('请输入混合模式')

    grid.addWidget(names[btn_name], mainUI.total_row - 1, 0)
    grid.addWidget(names[edit_name], mainUI.total_row - 1, 1)
